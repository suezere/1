...
本文将重点研究单无人机吊装载荷的高精度跟踪与多无人机协同稳定控制问题，探索数据驱动控制在无人机绳系吊运系统中的应用，并通过物理引擎仿真和实物试验验证其有效性。基于此研究框架，本文的具体研究内容如下：
...
针对多无人机协同吊运载荷的自适应稳定控制问题，基于分布式灵敏度传播的神经网络学习方法，设计了一种分布式策略梯度算法Autotune，用于自动调整系统的分布式模型预测控制参数。分布式灵敏度传播方法用于计算无人机和载荷的动力学的梯度并进行传播，分布式策略梯度算法采用深度神经网络在线生成自适应归一化超参数，以闭环形式有效地学习自适应模型预测控制中的权重参数，提高了多无人机绳系吊运系统的协调控制性能。

最后，搭建了无人机绳系吊运系统的物理引擎仿真平台和实物试验平台对本文所提出的算法进行了验证。在仿真过程中，首先使用了多种开源物理引擎对系统进行建模，基于开源物理引擎进行数值仿真，对单/多无人机绳系吊运系统进行了悬停测试、轨迹跟踪等，验证了本文提出算法的有效性；随后在实物试验中，提出了无人机绳系吊运系统的总体设计方案，并进一步细化了器件选型和机构设计，试验结果也验证了算法的有效性。
...
% 此外，多机器人运输方案中，很少有研究考虑起飞或着陆过程。尽管 Goodarzi 等人\cite{goodarzi2015dynamics} 以及Kumar等人\cite{michael2011cooperative}的实验表明其系统能够实现起降，但相关文献并未描述系绳在过渡阶段的数学建模。在这些阶段纳入系绳的动态特性会增加计算成本。
...
% 超过两个四旋翼的多升系统引入了系绳张力的冗余性[15]，导致张力在四旋翼之间的分布不唯一。在[16]–[18]中，作者提出了系绳张力分布的最小范数解法。相比之下，Geng等人[19][20]利用这种冗余性，在四旋翼的多升系统中实现了最佳张力分布，同时保证了安全的旋翼间隔，并考虑了四旋翼的推力限制。此外，最近的研究工作[21][22]将这种冗余性纳入了集中式MPC框架，不仅确保了安全约束，还通过充分考虑系统的非线性动力学，改善了系统的机动性。

% 如上述工作[13][19]–[23]所示，优化方法对于多升系统是有效的，因为它们能够管理约束条件和非线性动力学。然而，这些方法的性能在很大程度上取决于超参数的调节，包括目标函数中的加权和参考值。这些超参数的调节通常需要大量的人工努力，并且受到系绳之间复杂的动态相互连接的影响。为了简化这一过程，通常需要做出保守的假设，如将载荷视为点质量，假设为准静态飞行，并且张力分布均匀，这可能导致次优结果。随着四旋翼数量的增加以及当其最佳值是动态变化且相互关联时（例如，载荷的质量分布不均且系统必须动态调整配置以避开障碍物），手动选择这些参数的挑战会更大。
...
% 针对载荷未知的多无人机绳系吊运系统，自动调整控制器的参数并解决各无人机之间的系绳拉力分配问题。
...
%     (2)\textbf{安全间距约束}：$g_{j,k} = 2d_{\text{quad}} - \|p_{i,k} - p_{j,k}\|^2$；

%     (3)\textbf{避障约束}：$g_{{o-i},k}= d_{\text{quad}} + d_{\text{obs}} - \|p_{i,k} - p_{\text{obs}}\|^2$。
...
% 在多无人机绳系吊运系统中，由于无人机与载荷之间的系绳张力耦合以及无人机之间的间接耦合关系，系统的灵敏度传播变得复杂且计算成本较高。系绳引起的强动力学耦合作用使得每个智能体的闭环状态不仅受到自身控制器的影响，还会受到其他智能体控制器的影响。具体而言，所有无人机的动力学通过系绳张力与载荷直接耦合，而无人机之间的动力学则通过载荷间接耦合。因此，载荷控制器对每个无人机的动力学行为的影响通常要比其它无人机控制器的影响更为显著。为了有效解决这一问题，本小节提出了一种适用于多无人机系统的分布式灵敏度传播框架，用于高效调优控制器参数。

% 首先，定义一个一般的线性MPC问题：
% \begin{equation}
% 	\begin{aligned} 
% &\operatorname*{minimize}_{\bm{\tau}_{1:T}}& &  \sum_{t=1}^{T} \left( \frac{1}{2} \tau_t^\mathrm{T} C_t \tau_t + c_t^\mathrm{T} \tau_t \right)\\
% &\text{subject to}& & \begin{aligned}
% & x_{t+1} = F_t \tau_t + f_t \\
% & \bm{x}_{0} = \bm{x}_\text{init}\\
% & G_t \tau_t \leq l_t	\end{aligned}	
% \end{aligned}
% \label{LNMPC}
% \end{equation}

% 在这个问题中，\(\tau_t = \left[ x_{t} , u_{t}  \right]^\mathrm{T} \in \mathbb{R}^{n+m}\) 是需要优化的复合状态-控制变量；\(C_t \in \mathbb{R}^{(n+m) \times (n+m)}\) 和 \(c_t \in \mathbb{R}^{n+m}\) 是成本函数的参数；\(F_t = [A_t \, B_t] \in \mathbb{R}^{n \times (n+m)}\)；\(f_t \in \mathbb{R}^n\) 表示动力学中的残差项；\(G_t \in \mathbb{R}^{g \times (n+m)}\) 和 \(l_t \in \mathbb{R}^g\) 则表示状态和控制操作的约束条件。

% 问题（LMPC）是一个二次规划（QP）问题，可以通过现成的求解器高效地解决。获得最优解 \(\{ \tau_t^\ast \}_{t=1}^T\) 后，我们可以进一步求解以下辅助问题，以计算梯度 \(\frac{\partial u_1^\ast}{\partial [C_t]_{i,j}}, \frac{\partial u_1^\ast}{\partial [c_t]_{i}}, \frac{\partial u_1^\ast}{\partial [x_{\text{init}}]_{i}}\)：

% \begin{equation}
% 	\begin{aligned} 
% &\operatorname*{minimize}_{\tilde{\bm \tau}_{1:T}}& &  \sum_{t=1}^{T} \left( \frac{1}{2} \tilde{\tau}_t^\top C_t \tilde{\tau}_t + \tilde{c}_t^\top \tilde{\tau}_t \right)\\
% &\text{subject to}& & \begin{aligned}
% & \tilde{x}_{t+1} = F_t \tilde{\tau}_t \\
% & \tilde{x}_0 = \tilde{x}_{\text{init}}\\
% & \left[G_t\right]_{\mathcal{I}_{t,\text{act}}} \tilde{\tau}_t = 0	\end{aligned}	
% \end{aligned}
% \label{LMPC-Grad}
% \end{equation}
% 其中，\(\tilde{\tau}_t = \left[ \tilde{\bm x}_t , \tilde{\bm u}_t \right]^\mathrm{T} \in \mathbb{R}^{n+m}\) 是复合的辅助状态-控制量，需要优化；\([G_t]_{I_{t, \text{act}}}\) 表示在最优解 \(\tau_t^\ast\) 下激活的约束行，即 \(G_t \tau_t \leq l_t\) 的部分。

% \textbf{定理4.1:}记辅助问题（\ref{LMPC-Grad}）中的第一个最优控制动作为 \(\tilde{u}_1^\ast\)。通过求解式(\ref{LMPC-Grad})并使用表格I中列出的系数 \(\tilde{c}_t\) 和 \(\tilde{x}_{\text{init}}\)，可以获得（\ref{LNMPC}）的分析梯度。

% \textbf{证明：}
% 可以将（LMPC）的拉格朗日函数表示为：

% \[
% L(\tau, \lambda, \nu) = \sum_{t=1}^{T} \left( \frac{1}{2} \tau_t^\top C_t \tau_t + c_t^\top \tau_t \right) + \sum_{t=0}^{T-1} \lambda_t^\top \left( F_t \tau_t + f_t - x_{t+1} \right) + \sum_{t=1}^{T} \nu_t^\top \left( G_t \tau_t - l_t \right),
% \]
% 其中，\(\nu_t \in \mathbb{R}^g\)是与不等式约束相关的拉格朗日乘子。设 \(I_{t,\text{act}} \subseteq \{1, 2, \dots, g\}\) 表示不等式 \(G_t \tau_t \leq l_t\) 激活的行，即 \([G_t]_{I_{t,\text{act}}} \tau_t = [l_t]_{I_{t,\text{act}}}\)。对应地，\(I_{t,\text{ina}} = \{1, 2, \dots, g\} \setminus I_{t,\text{act}}\) 表示不激活的约束，即 \([G_t]_{I_{t,\text{ina}}} \tau_t < [l_t]_{I_{t,\text{ina}}}\)。

% 根据互补松弛性原理\cite{hindi2004tutorial}，在最优解时满足以下条件：
% \begin{equation}
% 	\begin{gathered}
% 	    [\nu_t]_{I_{t,\text{ina}}} = 0\\
%  \quad [G_t]_{I_{t,\text{ina}}} \tau_t - [l_t]_{I_{t,\text{ina}}} < 0\\
% [G_t]_{I_{t,\text{act}}} \tau_t - [l_t]_{I_{t,\text{act}}} = 0\\
% \quad [\nu_t]_{I_{t,\text{act}}} > 0	
% 	\end{gathered}
% \end{equation}
...
% \begin{equation}
% 	\begin{gathered}
% 		\lambda_{T-1}^* = C_{xx}^T \lambda_{T}^* + c_x^T + G_T^T \nu_T^*\\
% 		\lambda_t^* = F_{t+1}^T \lambda_{t+1}^* + C_{t+1,x}^T \lambda_{t+1}^* + c_{t+1,x} + G_t^T \nu_t^*
% 	\end{gathered}
% \end{equation}

% 我们可以将（LMPC）的KKT分解表示为式（33），其中 KKT 分解矩阵用 $\bm K_{LI}$ 表示。
...
% - x_{\text{init}} \\
% - c_1 \\
% 0_{\mathcal{I}_{1,\text{ina}}} \\
% \begin{bmatrix} l_1 \end{bmatrix}_{\mathcal{I}_{1,\text{act}}} \\
% - f_1 \\
% - c_2 \\
% 0_{\mathcal{I}_{2,\text{ina}}} \\
% \begin{bmatrix} l_2 \end{bmatrix}_{\mathcal{I}_{2,\text{act}}} \\
% \vdots \\
% - f_{T-1} \\
% - c_T \\
% 0_{\mathcal{I}_{T,\text{ina}}} \\
% \begin{bmatrix} l_T \end{bmatrix}_{\mathcal{I}_{T,\text{act}}}
% \end{bmatrix}
% \end{equation}
...
% \begin{bmatrix}
% - \text{d} x_{\text{init}} \\
% - \text{d} c_1 - \text{d} C_1 \tau_1^\star - \text{d} F_1^\top \lambda_1^\star - \text{d} G_1^\top \nu_1^\star \\
% 0_{\mathcal{I}_{1,\text{ina}}} \\
% - \text{d} \begin{bmatrix} G_1 \end{bmatrix}_{\mathcal{I}_{1,\text{act}}} \tau_1^\star + \text{d} \begin{bmatrix} l_1 \end{bmatrix}_{\mathcal{I}_{1,\text{act}}} \\
% \vdots \\
% - \text{d} f_{T-1} - \text{d} F_{T-1} \tau_{T-1}^\star \\
% - \text{d} c_T - \text{d} C_T \tau_T^\star - \text{d} G_T^\top \nu_T^\star \\
% 0_{\mathcal{I}_{T,\text{ina}}} \\
% - \text{d} \begin{bmatrix} G_T \end{bmatrix}_{\mathcal{I}_{T,\text{act}}} \tau_T^\star + \text{d} \begin{bmatrix} l_T \end{bmatrix}_{\mathcal{I}_{T,\text{act}}}
% \end{bmatrix}
% \end{equation}

% 从（34）中可以看出，考虑到对 \(C_t\) 和 \(c_t\) 的雅可比矩阵时，右侧非零的微分只有 \(dC_t\) 和 \(dc_t\)，

% 这一观察表明，\(\frac{\partial u_1^*}{\partial C_t}\) 和 \(\frac{\partial u_1^*}{\partial c_t}\) 的梯度可以通过求解以下辅助问题来获得（LMPC）的梯度：
% \begin{equation}
% 	\begin{aligned} 
% &\operatorname*{minimize}_{\tilde{\bm \tau}_{1:T}}& &  \sum_{t=1}^T\frac{1}{2}\tilde{\tau}_t^\top\tilde{C}_t\tilde{\tau}_t+\tilde{c}_t^\top\tilde{\tau}_t\\
% &\text{subject to}& & \begin{aligned}
% & \tilde{x}_{t+1} = F_t \tilde{\tau}_t \\
% & \tilde{x}_0 = \tilde{x}_{\text{init}}\\
% & \tilde{G}_t\tilde{\boldsymbol{\tau}}_t=\tilde{\boldsymbol{l}}_t	\end{aligned}	
% \end{aligned}
% \label{LMPC-Aux}
% \end{equation}

% 特别地，为了获得梯度 \(\frac{\partial u_1^*}{\partial [C_t]_{i,j}}\)，只需要求解（\ref{LMPC-Aux}）问题，其中：
% $
% \tilde{C}_t = C_t$， $ \tilde{c}_t = [(\tau_1^*)^\top \otimes I_{n+m}]_{(i-1)(n+m)+j} = [\tau_t^*]_j e_{n+m}^i$， $ f_t = 0$, $ \tilde{x}_{\text{init}} = 0$， $\tilde G_t = [G_t]_{I_{t,\text{act}}}$， $\quad \tilde{l}_t = 0$
% 同样地，通过这种方式可以计算其他梯度。
...
%     \begin{algorithmic}[1]
%     \State $k \gets 1$
%     \While{$e \geq \delta$ $\textbf{and}$ $k \leq k_{\text{max}}$}
%         \For{$i \gets 1$ $\bm{\textbf{to}}$ $n$ \textbf{(并行)}}
%             \For{$t \gets T$ $\bm{\textbf{to}}$  $T+N$}
%                 \State 计算灵敏度轨迹：$X_{i,t}^i$, $X_{i,t}^l$, $X_{l,t}^i$
%                 \Statex \quad \quad \quad \quad \ \  基于载荷的灵敏度 $X_{l,t}^{l,k-1}$
%                 \Statex \quad \quad \quad \quad \ \ $\triangleright$ 在每个无人机中运行，并发送至中央无人机
%             \EndFor
%         \EndFor
...
%         \State 计算误差: $e \gets \max \{ e_i^i, e_i^l, e_l^i, e_l^l \}, \forall i \in \mathcal{I}_q$
%         \Statex \quad \ \ 其中:
...
%     \end{algorithmic}
% \end{algorithm}
% 具体而言，所有无人机的动力学通过系绳张力与载荷直接耦合，而无人机之间的动力学则通过载荷间接耦合。因此，载荷控制器对每个无人机的动力学行为的影响通常要比其它无人机控制器的影响更为显著。



% \subsection{分布式策略梯度学习算法}

% 通过分布式灵敏度传播算法，可以高效地获得系统的灵敏度矩阵。这些灵敏度进一步用于分布式策略梯度学习优化方法，以实现对控制器参数的高效调优。
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        \While{$L_{\text{mean}}$ 未收敛}
            \For{$k = 0$ \textbf{to} $T$ \textbf{by} $\Delta \bar t$ }
                \State \textbf{前向传播:}
...
% 在本节中，通过广泛的仿真实验验证了的有效性，实验主要针对不同飞行场景下模型预测控制（MPC）超参数的学习。

% \begin{enumerate}
%     \item \textbf{快速收敛}：所提出的\textbf{分布式灵敏度传播}算法在仅需少量迭代的情况下即可实现快速收敛，且收敛速度不受无人机数量的影响。  

%     \item \textbf{自适应MPC权重学习}：能够根据轨迹跟踪误差直接学习自适应的MPC权重，与现有的开环学习方法相比，大幅提升了训练稳定性和跟踪性能。  

%     \item \textbf{动态障碍物穿越}：除了改进MPC权重学习能力外，方法还能够有效学习自适应张力参考值，使多吊点系统在穿越障碍物时能够进行动态重构。
% \end{enumerate}

% \subsection{自适应MPC超参数的约束与网络架构}

% 为了确保MPC超参数 \( \theta_i \) 在物理上合理且计算上受限，采用以下参数化形式：
% \begin{equation}
...
% 为了生成在线自适应的归一化超参数 \( \Theta_i \)，使用多层感知器（MLP）网络。该网络的输出层采用\textbf{Sigmoid激活函数}，确保输出范围在 \([0, 1]\) 之间。此外，网络包含两个隐藏层，使用\textbf{ReLU激活函数}，并在线性层中应用\textbf{谱归一化}，以约束层的Lipschitz常数，从而提高网络的鲁棒性和泛化能力~\cite{ref42}。
...
% 这里的计算的方法通过Python实现，分布式MPC问题（如算法1所示）使用\textbf{CasADi}框架中的ipopt求解~\cite{Andersson2019}。MLP网络则通过\textbf{PyTorch}构建，并使用Adam优化器进行训练~\cite{ref44, ref45}。

% 在训练过程中，自定义损失函数 \( L_i^\text{pytorch} \) 以适配PyTorch的训练流程，定义如下：
% \begin{equation}
