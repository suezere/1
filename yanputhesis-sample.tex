...
\makeCommitteePage{                                         % 学位论文评阅人
    ...
    \committee{2025 年 3 月 6 日}{
        \defenseChair{刘正雄}{教授}{西北工业大学}
        \committeeMember{马志强}{副教授}{西北工业大学}
...
为解决使用四旋翼无人机和系绳进行协同吊装运输载荷的难题，Maza 等人\cite{maza2010multi}设计了一个PID控制器。然而，该控制器方法无法有效减小欠阻尼系统中的振荡。Bacelar 等人\cite{bacelar2020board}开发了一种基于两架无人机的协同载荷运输控制方法，结合了线性二次调节控制器与卡尔曼滤波器。该方法通过两架配备超声波高度计、惯性测量单元和前置摄像头的商用四旋翼无人机进行了试验验证。
...
Tognon等人\cite{tognon2018aerial}提出了一种基于李雅普诺夫的分布式控制器，用于两架无人机对系绳吊挂载荷的空中操控。该工作的目标是在不依赖主从阻抗控制器进行通信的情况下，调节载荷的位置和姿态。
...
\begin{equation}
	\Vert {E_n} \Vert \le \sum_{i=0}^{n-1}\Vert \tilde{\mathcal{K}}_d^i\Vert \cdot \Vert \bm{e}(t_0+(n-i)t_s) \Vert\le \Vert c \Vert \sum_{i=0}^{n-1}\Vert {\mathcal{K}}_d^i \Vert
\end{equation}
...
(3)提出了一种自适应学习的超参数优化算法——Autotune，自动调节多无人机绳系吊运系统中的各种MPC超参数，其中所有超参数均通过深度神经网络（DNN）表示。引入了分布式灵敏度传播算法，充分利用了多无人机系统中的动态耦合特性，并行计算无人机上的梯度。在此基础上，使用分布式策略梯度学习算法生成归一化超参数，并整合到分布式模型预测控制框架中，从而实现了控制器的实时动态适应能力。
...
\end{equation}
其中，\( \bm \Theta_i \) 为归一化超参数，范围在 \([\text{0}, \text{1}]\) 之间，\(\bm{\theta}_{i,\text{min}}\)和\(\bm{\theta}_{i,\text{max}}\)分别表示超参数的物理下限与上限，其确定方法根据超参数类型不同而有所区别：
当\(\bm{\theta}_i\)为MPC代价函数权重时，下界\(\bm{\theta}_{i,\text{min}}\)能够保证对角权重矩阵的正定性，上界\(\bm{\theta}_{i,\text{max}}\)通过限制权重值的无界增长，有效避免数值发散问题；当\(\bm{\theta}_i\)为自适应MPC的参考轨迹（如张力参考）时，下界\(\bm{\theta}_{i,\text{min}}\)和上界\(\bm{\theta}_{i,\text{max}}\)则直接由电机最大扭矩、机械结构安全阈值等物理约束确定，从而保证参考轨迹的动态可行性。
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
其中，$L_l $和 $L_i$分别表示载荷和第 \(i\) 个无人机的代价函数，分别通过超参数\(\bm{\theta}_l\)与\(\bm{\theta}_i\)实现动态权重分配，基于它们在给定预测时间范围 \( N \in \mathbb{R}^+ \) 内的状态变化计算。参数 \(\bm \pi \) 包含了所有无人机的DNN参数 \( \bm \pi_i \)和载荷的DNN参数 \( \bm \pi_l \)，
...
%其中，代价函数\( L_l(\bm{\theta}_l) \)和\( L_i(\bm{\theta}_i) \)分别通过超参数\(\bm{\theta}_l\)与\(\bm{\theta}_i\)实现动态权重分配。具体而言：
%\begin{itemize}
%	\item 无人机代价函数\( L_i(\bm{\theta}_i) \)包含跟踪误差项\(\|\bm{x}_i - \bm{x}_i^{\text{ref}}\|_{\bm{Q}_i}^2\)与控制量惩罚项\(\|\bm{u}_i\|_{\bm{R}_i}^2\)，其中权重矩阵\(\bm{Q}_i, \bm{R}_i\)由\(\bm{\theta}_i\)在线更新
%	\item 载荷代价函数\( L_l(\bm{\theta}_l) \)考虑负载摆角抑制\(\|\alpha_l\|_{q_l}^2\)与能量消耗\(\|\bm{u}_l\|_{r_l}^2\)，其权重系数\( q_l, r_l \)通过\(\bm{\theta}_l\)自适应调整
%\end{itemize}

%\textbf{关键改进}：通过将超参数\(\bm{\theta}\)嵌入代价函数结构，原有人工设定的固定权值比例被转化为可学习参数的空间映射问题。具体表现为：
%\begin{itemize}
%	\item 权重自主调节：超参数\(\bm{\theta}_i\)与\(\bm{\theta}_l\)通过梯度下降更新，使得\( L_i \)与\( L_l \)的相对重要性随系统动态变化自动平衡
%	\item 耦合关系解耦：无人机-载荷交互作用被编码在状态约束\( f_{i,k}, f_{l,k} \)中，而代价函数权重通过\(\bm{\theta}\)的独立参数化实现分布式优化
%	\item 数值鲁棒性提升：基于第3.2节的参数化方法，\(\bm{\theta}\)的可行域约束保证权重系数始终处于正定且物理合理的范围
%\end{itemize}
...
\autoref{cjb} 以3D图的方式展示了吊运载荷的轨迹跟踪过程。相较于开环训练的MPC方法，Autotune-MPC在收敛速度方面表现出显著优势，实验数据显示，Autotune-MPC在训练周期内将x轴和y轴上载荷的轨迹跟踪均方根误差（RMSE）从0.038 m和0.196 m降低至0.026m和0.041 m，最大跟踪误差减少13.6\%。通过实时校正动力学模型偏差，Autotune-MPC在Z轴方向的载荷轨迹跟踪均方根误差为0.002 m，较开环MPC方法有了显著的提升。这种性能优势主要源于Autotune-MPC直接在实际系统动力学的闭环系统状态上进行训练的特征，而传统方法依赖于名义动力学中预测得到的开环状态。

从\autoref{cjb1} 中可以看出，MPC计算的拉力比实际拉力平滑得多，在Autotune-MPC中，使用MPC计算的得到的拉力代替实际拉力，解决了实际拉力和MPC计算拉力之间的差异。
...
(4)仿真与试验验证：
为验证所提算法的有效性，本文搭建了无人机绳系吊运系统的物理引擎仿真平台，选用Drake、Gazebo和MuJoCo等开源物理引擎对无人机吊运系统进行建模与仿真。结果表明，所提出的数据驱动控制方法能够有效跟踪期望轨迹，并在复杂环境中保持系统稳定性。此外，本文进一步搭建了无人机吊运系统的飞行试验平台，通过实物试验验证了算法在实际应用中的可行性与鲁棒性。
...
\begin{accomplishments}                                     % 参加科研情况开始
	论文发表情况：
	\begin{enumerate}
		\item \blackbox{Ao Jin}, \blackbox{Chenhao Li}, \blackbox{Ya Liu}， \blackbox{Panfeng Huang} and \blackbox{Fan Zhang}. Neural Predictor for Flight Control with Payload. 
		arXiv preprint arXiv:2410.15946, 2024.
	\end{enumerate}
