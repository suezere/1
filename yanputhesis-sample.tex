...
\subsection{升维线性系统}
Koopman算子可以从数据中将非线性系统全局映射为线性系统 \cite{Mamakoukas2023}。这一性质为以数据驱动的方式显式捕捉未知非线性动力学（如式 (\ref{3-1}) 中的$\bm f_e$和$\bm \tau_e$）提供了途径。因此，本文采用这一特性，通过数据驱动的方式学习$\bm f_e$和$\bm \tau_e$。升维线性系统电基本概念简述如下：
...
无人机与载荷的离散动态方程，载荷张力的虚拟控制 \( \bm u_l \) 用作优化变量；初始条件；
无人机的控制约束；
系绳张力约束，用于保持系绳绷紧；
...
\textbf{注：}为了有效提升收敛性，将式 (\ref{multimpc}) 中的约束转换为软约束，并将其引入到目标函数中。同时，可以采用障碍函数来表述软约束，障碍参数足够小的情况下可以逼近原始约束问题。障碍参数 $\gamma > 0$ 控制约束满足程度和计算效率之间的权衡。
...
\begin{equation}
	\begin{aligned} 
	&\operatorname*{minimize}_{\bm{X}_l, \bm{U}_l}& & \min_{\bm{X}_l, \bm{U}_l} J_l + \sum_{k=0}^{N} \left( \frac{1}{2\gamma} \sum_{i=1}^{n} h_{i,k}{}^2 - \gamma \ln(-g_{{o-l},k}) \right)\\
	&\text{subject to}& & \begin{aligned}
...
对应的 模型预测控制 预测轨迹分别记为
$\xi_{{\theta_i},{*,i}} = \{ X_{{\theta_i},{*,i}}, U_{{\theta_i},{*,i}} \}$和
$\quad \xi_{{\theta_i},{*,i}} = \{ X_{{\theta_i},{*,i}}, U_{{\theta_i},{*,i}} \}$。
...
\subsection{控制器超参数自适应调节方法}
考虑一个离散时间的动力学系统:

\begin{equation}
    \bm x_{t+1} = T(\bm x_t, \bm u_t)
	\label{1}
\end{equation}
...
\begin{equation}
    \bm u_k = h(\bm x_k, \bm x_{t,\text{ref}}, \bm \theta)
	\label{2}
\end{equation}

其中 \( \bm \theta \in \bm \Theta \) 表示控制器的参数，\( \bm \Theta \) 是一个可行的参数集合，可以通过分析或经验确定系统的稳定性。  
假设状态 \( {\bm x}_k \) 可以直接测量，或者使用适当的状态估计器。此外，假设式(\ref{1}) 和式 (\ref{2}) 是可微的，即雅可比矩阵$\nabla_{\bm x} T$， $\ \bm \nabla_{\bm u} T$， $\ \bm \nabla_{\bm x} h$ 和 $\ \bm \nabla_{\bm \theta} h$
都存在，这对一般系统广泛适用。

控制器学习/自调任务调整参数 \( \bm \theta \) 以最小化评估准则，即损失函数 \( L(\cdot) \)，该损失函数是目标状态、实际状态和控制行为在时间间隔 \( N \) 内的函数。  
...
\]
其中 \( \lambda > 0 \) 是惩罚系数。在本文的其余部分，我们将使用简写符号 \( L(\bm \theta) \) 来表示损失函数。

总结一下，我们将控制器学习/自调问题表述为以下参数优化问题：
\[
\min_{\theta \in \Theta} L(\theta) \quad \text{subject to} \ x_{k+1} = f(x_k, u_k), \ x_0 \ \text{given}, \ u_k = h(x_k, \bar{x}_k, \theta), \ k \in \{0, 1, \dots, N-1\}. \tag{P}
\]

我们通过 DiffTune 来求解上述问题，DiffTune 使用投影梯度下降算法来迭代更新参数：
\begin{equation}
    \bm \theta \leftarrow P_{\bm \Theta} (\bm \theta - \alpha \bm{\nabla_\theta L})
\end{equation}

为了得到梯度 \( \bm{\nabla_\theta L} \)，从链式法则出发：
\begin{equation}
    \bm{\nabla_\theta L} = \sum_{k=1}^{N} \frac{\partial L}{\partial \bm x_k} \frac{\partial \bm x_k}{\partial \bm \theta} + \sum_{k=0}^{N-1} \frac{\partial L}{\partial \bm u_k} \frac{\partial \bm u_k}{\partial \bm \theta}
\end{equation}

一旦选择了损失函数 \( L \)，就可以确定梯度 \( \frac{\partial L}{\partial \bm x_k} \) 和 \( \frac{\partial L}{\partial \bm u_k} \)，并通过灵敏度传播获得灵敏度状态 \( \frac{\partial \bm x_k}{\partial \bm \theta} \) 和 \( \frac{\partial \bm u_k}{\partial \bm \theta} \)：
\begin{equation}
	\begin{gathered}
		\frac{\partial \bm x_{k+1}}{\partial \bm \theta} = (\bm{\nabla_{x_k}}  T + \bm{\nabla_{u_k}} T \bm{\nabla_{x_k}} h) \frac{\partial \bm x_k}{\partial \bm \theta} + \bm{\nabla_{u_k}} T \bm{\nabla_{\theta}} h\\
		\frac{\partial \bm u_k}{\partial \bm \theta} = \bm{\nabla_{x_k}} h \frac{\partial \bm x_k}{\partial \bm \theta} + \bm{\nabla_{\theta}} h
	\end{gathered}
\end{equation}

其中 \( \frac{\partial \bm x_0}{\partial \bm \theta} = 0 \)。  
这里符号 \( \bm{\nabla_{x_k}} T, \bm{\nabla_{u_k}} T, \bm{\nabla_{x_k}} h, \bm{\nabla_{\theta}} h \) 表示在状态 \( \bm x_k \) 和控制 \( \bm u_k \) 处对函数 \( T \) 和 \( h \) 的雅可比矩阵求导。

\subsection{基于梯度下降的控制器超参数自适应调节方法}

由于多无人机绳系吊运系统的动态行为复杂，难以通过解析模型精确描述，我们引入深度神经网络（DNN）生成控制器的动态权重和参考轨迹。DNN 的输入特征包括系统状态、障碍物位置等环境信息。第 \(i\) 个无人机的自适应模型预测控制超参数和载荷的自适应模型预测控制超参数可以表示为：

\begin{equation}
	\label{4-7}
	\begin{gathered}
		\boldsymbol{\theta}_i=f_{\boldsymbol{\pi}_i}\left(\boldsymbol{\chi}_i\right),\mathrm{~}\forall i\in\mathcal{I}_q\\    
		\boldsymbol{\theta}_l=f_{\boldsymbol{\pi}_l}\left(\boldsymbol{\chi}_l\right)
	\end{gathered}
\end{equation}
...
\begin{equation}
	\label{4-8}
	\begin{aligned} 
		&\operatorname*{minimize}_{\bm{\pi}}& & L(\bm \theta_l)+\sum_{i=1}^n L(\bm \theta_i) \\
		&\text{subject to}& & \begin{aligned}
			&\boldsymbol{x}_{i,{t+1}}=\bar{{f}}_{i,t}\left(\boldsymbol{x}_{i,t},\boldsymbol{u}_{{*,i},{0|t}},\Delta t;\boldsymbol{x}_{l,t},\boldsymbol{u}_{{*,i},{0|t}}\right),\forall i\in\mathcal{I}_q\\&\boldsymbol{x}_{i,{t+1}}=\bar{{f}}_{l,t}\left(\boldsymbol{x}_{l,t},\boldsymbol{u}_{{*,l},{0|t}},\Delta t;\boldsymbol{x}_{i,t}\right),\forall i\in\mathcal{I}_q\\
		\end{aligned} \\
	\end{aligned}
\end{equation}
其中，$L(\bm \theta_l)$和 $L(\bm \theta_i)$分别表示载荷和第 \(i\) 个无人机的损失函数，损失基于它们在给定预测时间范围 \( N_c \in \mathbb{R}^+ \) 内的状态变化计算。参数 \( \pi \) 包含了所有无人机的DNN参数 \( \pi_i \)，
\( \boldsymbol{u}_{*,i,{0|t}} \)
由第 \(i\) 个无人机的模型预测控制器生成在时刻 \(t\) 的最优控制输入\( \boldsymbol{\theta}_i \) ，\( \boldsymbol{u}_{*,l,{0|t}} \)由载荷的模型预测控制器生成在时刻 \(t\) 的最优控制输入(\ \boldsymbol{\theta}_l \)。
...
为了突出闭环训练的优越性，我们将其与现有的开环训练方法进行比较。在保持闭环训练中的模型预测控制设置一致的情况下，继续使用式 (\ref{4-7}) 中定义的自适应模型预测控制超参数，可以将多机协同提升系统的开环训练问题描述为以下形式：
...
在式(\ref{4-10})中，第一项和第三项的梯度可以直接计算，因为个体损失函数和模型预测控制超参数均是闭环状态和DNN参数的显函数。主要挑战在于求解第二项，即闭环状态对模型预测控制超参数的梯度。为了提高运算效率，下一小节将使用分布式灵敏度传播方法，将计算任务分解到各无人机和载荷中并行执行。
